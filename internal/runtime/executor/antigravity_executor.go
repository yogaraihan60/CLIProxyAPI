...
// normalizeModelForRequest strips image model suffixes and returns the base model name
// for use in API requests. This allows models like "gemini-3-pro-image-preview-4k"
// to be sent as "gemini-3-pro-image-preview" to the backend.
func normalizeModelForRequest(modelName string) string {
	imgConfig := util.ParseImageModelSuffixes(modelName)
	return imgConfig.BaseModel
}
